...
import { Mail, Lock, User, Eye, EyeOff, AlertCircle, CheckCircle2, ArrowRight, Check } from 'lucide-react';
    ...
    <div className={`min-h-screen flex items-center justify-center ${bgPrimary} py-12 px-4 sm:px-6 lg:px-8`}>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className={`text-3xl font-bold ${textPrimary} mb-2`}>Create Account</h1>
          <p className={textSecondary}>Join thousands of smart investors today</p>
        </div>
        ...
        <div className={`${bgSecondary} border ${borderColor} rounded-xl p-6 sm:p-8 space-y-6`}>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Input
                  ...
                  placeholder="John"
                  className={`${isDark ? 'bg-[#1a1f2e] border-[#2a2e39]' : 'bg-gray-50'} ${textPrimary}`}
                />
              </div>
              <div>
            ...
            <div className="space-y-2 p-3 rounded-lg bg-slate-500/5 border border-slate-500/10">
              <p className={`text-xs font-medium ${textSecondary}`}>Password must contain:</p>
              <div className="space-y-1">
                {[
                  ...
                  <div key={item.key} className="flex items-center gap-2">
                    <Check
                      className={`w-4 h-4 ${passwordStrength[item.key] ? 'text-emerald-400' : 'text-gray-500'}`}
                    />
                    <span className={`text-xs ${passwordStrength[item.key] ? 'text-emerald-400' : textSecondary}`}>
                      {item.label}
                    </span>
                  </div>
                ))}
            ...
            <Button
              ...
              className="w-full bg-cyan-500 hover:bg-cyan-600 text-white py-2.5 rounded-lg font-medium transition-colors flex items-center justify-center gap-2 disabled:opacity-50"
            >
