...
export default function TimeRangeSelector({ selected, onSelect, isDark = true }) {
  return (
    <div className={`flex items-center gap-0.5 p-1 rounded-xl flex-shrink-0 ${isDark ? 'bg-[#1a1f2e]' : 'bg-gray-100'}`}>
      {ranges.map((range) => (
        <button
          key={range}
          onClick={() => onSelect(range)}
          className={`
            px-3 py-1.5 rounded-lg text-xs font-semibold transition-all duration-200 flex-shrink-0 whitespace-nowrap
            ${selected === range 
